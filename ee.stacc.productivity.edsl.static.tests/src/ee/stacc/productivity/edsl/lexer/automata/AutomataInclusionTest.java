...
public class AutomataInclusionTest {
	...
	public void testInclusion() throws Exception {
		...
		assertTrue(AutomataInclusion.INSTANCE.checkInclusion(AutomataParser
				.parse(automaton1).getInitialState(), AutomataParser.parse(
		...
		assertTrue(AutomataInclusion.INSTANCE.checkInclusion(
				AutomataDeterminator.determinate(AutomataParser.parse(
						automaton1).getInitialState()), AutomataDeterminator
		...
		automaton2 = "A1 - A4:S !A5:V A1:V;" + "A4 - A4:S !A5:V;" + "!A5;";
		assertTrue(AutomataInclusion.INSTANCE.checkInclusion(AutomataParser
				.parse(automaton1).getInitialState(),
				AutomataDeterminator.determinate(AutomataParser.parse(
						automaton2).getInitialState())));
	...
	public void testTransduction() throws Exception {
		...
		transduction = AutomataInclusion.INSTANCE.getTrasduction(transducer
				.getInitialState(), automaton.getInitialState());
		assertTrue(AutomataInclusion.INSTANCE.checkInclusion(check
				.getInitialState(), transduction));
		assertTrue(AutomataInclusion.INSTANCE.checkInclusion(transduction,
				check.getInitialState()));

		// 0*_+("")*0
		automatonStr = "A1 - A1:0 A2:_;" + "A2 - A2:_ A3:\" !A5:0;"
				+ "A3 - A4:\";" + "A4 - A3:\" !A5:0";
		// (0+/V | "a*"/S | _)*
		transducerStr = "!T1 - !T1:_ !T2:0/V T3:\" ;"
				+ "!T2 - !T2:0 !T1:_ T3:\" ;" + "T3 - T3:a !T1:\"/S ;";
		// V*S*V
		checkStr = "C1 - C1:V C2:S !C3:V;" + "C2 - C2:S !C3:V;";

		automaton = AutomataParser.parse(automatonStr);
		transducer = AutomataParser.parse(transducerStr);
		check = AutomataParser.parse(checkStr);
		State checkInit = AutomataDeterminator.determinate(check
				.getInitialState());
		transduction = AutomataInclusion.INSTANCE.getTrasduction(transducer
				.getInitialState(), automaton.getInitialState());
		...
		assertTrue(AutomataInclusion.INSTANCE.checkInclusion(transduction,
				checkInit));
		assertTrue(AutomataInclusion.INSTANCE.checkInclusion(checkInit,
				transduction));
	...
	public void testLoops() throws Exception {
		...
		IAbstractString str = new StringSequence(new StringConstant("SELECT "),
				new StringRepetition(new StringCharacterSet("a1.")),
